...
public class TestHsWebServicesLogs extends JerseyTestBase {
      ...
      try {
        doAnswer(invocationOnMock -> {
          ...
          if (request.getApplicationId().equals(APPID_1)) {
            return GetApplicationReportResponse.newInstance(newApplicationReport(APPID_1, APP_ATTEMPT_1_2, false));
          } else if (request.getApplicationId().equals(APPID_2)) {
            return GetApplicationReportResponse.newInstance(newApplicationReport(APPID_2, APP_ATTEMPT_2_2, true));
          }
        ...
        doAnswer(invocationOnMock -> {
          ...
          if (cId.equals(CONTAINER_2_2_2)) {
            return GetContainerReportResponse.newInstance(newContainerReport(cId, NM_ID_1, NM_WEBADDRESS_1));
          } else if (cId.equals(CONTAINER_2_2_3)) {
            return GetContainerReportResponse.newInstance(newContainerReport(cId, NM_ID_2, NM_WEBADDRESS_2));
          }
          return GetContainerReportResponse.newInstance(newContainerReport(cId, null, null));
        }).when(mockProtocol).getContainerReport(any());
      } catch (Exception ignore) {
        fail("Failed to setup WebServletModule class");
      }

      HsWebServices hsWebServices = new HsWebServices(appContext, conf, webApp, mockProtocol);
      try {
  ...
  private static void createAggregatedFolders() throws Exception {
    Map<ContainerId, String> contents_app1 = new HashMap<>();
    contents_app1.put(CONTAINER_1_1_1, "Hello-" + CONTAINER_1_1_1);
    contents_app1.put(CONTAINER_1_1_2, "Hello-" + CONTAINER_1_1_2);
    contents_app1.put(CONTAINER_1_2_1, "Hello-" + CONTAINER_1_2_1);

    TestContainerLogsUtils.createContainerLogFileInRemoteFS(conf, fs,
        rootLogDir, APPID_1, contents_app1, NM_ID_1, FILE_NAME, USER, false);

    TestContainerLogsUtils.createContainerLogFileInRemoteFS(conf, fs,
        rootLogDir, APPID_1, Collections.singletonMap(CONTAINER_1_1_3,
            "Hello-" + CONTAINER_1_1_3), NM_ID_2, FILE_NAME, USER, false);

    Map<ContainerId, String> contents_app2 = new HashMap<>();
    contents_app2.put(CONTAINER_2_1_1, "Hello-" + CONTAINER_2_1_1);
    contents_app2.put(CONTAINER_2_2_1, "Hello-" + CONTAINER_2_2_1);

    TestContainerLogsUtils.createContainerLogFileInRemoteFS(conf, fs,
        rootLogDir, APPID_2, contents_app2, NM_ID_1, FILE_NAME, USER, false);

    TestContainerLogsUtils.createContainerLogFileInRemoteFS(conf, fs,
        rootLogDir, APPID_2, Collections.singletonMap(CONTAINER_2_2_3,
  ...
  public void testGetAggregatedLogsMetaForFinishedApp() {
    ...
    assertThat(responseList.size()).isEqualTo(expectedIdStrings.size());
    ...
    for (ContainerLogsInfo logsInfo : responseList) {
      ...
      if (cId.equals(CONTAINER_1_1_3.toString())) {
        assertThat(logsInfo.getNodeId()).isEqualTo(NM_ID_2.toString().replace(":", "_"));
      } else {
        assertThat(logsInfo.getNodeId()).isEqualTo(NM_ID_1.toString().replace(":", "_"));
      }
  ...
  public void testGetAggregatedLogsMetaForRunningApp() {
    ...
    Set<String> actualStrings =
        responseList.stream()
            ...
            .collect(Collectors.toSet());
    assertThat(actualStrings).isEqualTo(expectedIdStrings);
    assertThat(responseList.size()).isEqualTo(expectedIdStrings.size() * 2);
  ...
  public void testGetContainerLogsForFinishedContainer() {
    ...
    assertThat(logsInfo.getContainerLogsInfo().size()).isOne();
    ContainerLogFileInfo fileInfo = logsInfo.getContainerLogsInfo().get(0);
    assertThat(fileInfo.getFileName()).isEqualTo(FILE_NAME);
    assertThat(fileInfo.getFileSize()).isEqualTo(
        String.valueOf(("Hello-" + CONTAINER_1_1_2).length()));
  }
  ...
  public void testGetContainerLogsForRunningContainer() throws Exception {
    ...
    if (logsInfo1.getLogType().equals(
      ...
      assertThat(logsInfo1.getContainerLogsInfo().size()).isOne();
      ContainerLogFileInfo fileInfo1 = logsInfo1.getContainerLogsInfo().get(0);
      assertThat(fileInfo1.getFileName()).isEqualTo(FILE_NAME);
      assertThat(fileInfo1.getFileSize()).isEqualTo(
          String.valueOf(("Hello-" + CONTAINER_2_2_3).length()));
      ...
      assertThat(logsInfo2.getContainerLogsInfo().size()).isOne();
      ContainerLogFileInfo fileInfo2 = logsInfo1.getContainerLogsInfo().get(0);
      assertThat(fileInfo2.getFileName()).isEqualTo(FILE_NAME);
      assertThat(fileInfo2.getFileSize()).isEqualTo(
          String.valueOf(("Hello-" + CONTAINER_2_2_3).length()));
    }
  }
  ...
  public void testGetContainerLogFileForFinishedContainer() {
    ...
    ClientResponse response = r.path("ws").path("v1")
        ...
        .path(CONTAINER_1_1_2.toString()).path("syslog")
  ...
  private static ApplicationReport newApplicationReport(ApplicationId appId,
    ...
    return ApplicationReport.newInstance(appId, appAttemptId, USER,
        ...
        running ? YarnApplicationState.RUNNING : YarnApplicationState.FINISHED, "fake an application report", "",
        1000L, 1000L, 1000L, null, null, "", 50f,
        "fakeApplicationType", null);
  }

  private static ContainerReport newContainerReport(ContainerId containerId, NodeId nodeId, String nmWebAddress) {
