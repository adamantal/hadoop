...
Rules are evaluated from top to bottom. When the evaluation stops and what happens if a rule doesn't match can be adjusted more flexibly compared to the legacy mapping rule evaluator.
...
| `parentQueue` | In case of `user`, `primaryGroup`, `primaryGroupUser`, `secondaryGroup`, `secondaryGroupUser`, this tells the engine where the matching queue should be looked for. For example, if the policy is `primaryGroup`, parent is `root.groups` and the submitter's group is "admins", then the resulting queue will be "root.groups.admin" |
| `fallbackResult` | If the target queue doesn't exist or it cannot be created, it defines a fallback action. Valid values are `skip`, `reject` and `placeDefault`. |
| `create` | Only applies to managed queue parents. If set to "false", then the queue will not be created if it doesn't exist. |
 ...
 * Policies
  ...
  There are a number of pre-defined placement policies which are similar to those in Fair Scheduler. Many of them can be expressed as a "custom" placement policy as you will soon see, but in many cases, it's safer and more straightforward to use them directly.

| Policy | Description |
|:---- |:---- |
| `specified` | Places the application to the queue that was defined during submission. The is the same as `<specified>` in Fair Scheduler. |
| `reject` | Rejects the submission. |
| `defaultQueue` | Places the application into the default queue `root.default`, but this can be changed. |
...
| `primaryGroupUser` | Places the application into the queue hiearchy `root.[parentQueue].<primaryGroup>.<userName>`. Note that `parentQueue` is optional. |
| `secondaryGroup` | Places the application into a queue which matches the secondary group of the submitter. |
| `secondaryGroupUser` | Places the application into the queue hiearchy `root.[parentQueue].<secondaryGroup>.<userName>`. Note that `parentQueue` is optional. |
| `setDefaultQueue` | Changes the default queue from `root.default`. The change is permament in a sense that it is not restored in the next rule. |
| `custom` | Enables the user to use custom placement strings. See explanation below. |

Notes:

1. The `setDefaultQueue` rule only changes the default queue. If you want to restore the default queue back to `root.default`, then it has to be added to the chain again.
...
| Old mapping rule | JSON-based rule |
|:---- |:---- |
...
5. If there is no such queue, take the queue from the application submission context.

6. If none of the above matches, then the application should be placed to `root.default`.

7. If the default placemene fails for whatever reason, we change the default queue to `root.users.default`.
